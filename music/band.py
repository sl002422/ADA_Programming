...
# %%
class Band: # we can skip () in Band() here
    ...
    genres = ['rock', 'blues', 'punk']  # universal attributes for all the object of this class
    ...
    def __iter__(self):  # this method defines iterator for the class
        ...
        self.__i = 0    # we can define it in the main, but we could do it here
...
print(Band.genres)
the_beatles = Band('The Beatles', *[johnLennon, paulMcCartney, georgeHarrison, ringoStarr], date(1957, 7, 6),
                   date(1970, 4, 11))
the_rolling_stones = Band('The Rolling Stones')
the_rolling_stones.genres.append('alternative')  # look carefully, it'll chenge it in the CLASS object, means for
# every object of the BAND it will be changed:
...
# print(next(ge))     # StopIteration will appear here
...
# %%
# python suggests to develop two classes so we do:
...
# l = [1, 3, 4, 5]      # index error
# for i in range(4):
#     print(l[i])

# d = {'a': 1, 'b': 2}
# print(d['c'])  # it is an exceptional situation, it should run somehow and the program should not break,
# this is why we use exceptions

# while using exceptions python creates an object if a problem occurs that describes what actually happened
...
try:
    ...
    # sys.stderr.write('Caught and exception')  # or print('Caught and exception')
    # sys.stderr.write(f'{type(e)}, {e.args[0]}')   # any exception object is a tuple, so now we will have the exact error
    sys.stderr.write(f'\n{type(e).__name__}: {e.args[0]}\n\n') #extracts name from a class
print('Done')   # done can be printed before, that is unpredictable as their run in different threads
...
try:
    ...
    print('Caught an exception. Stopped any further processing.')       # no matter what happens before, finally is executed anyway
...
try:
    ...
    #sys.stderr.write(f'{type(e).__name__}, {e.args[0]}')
# else:
#     print('Whatever')     # it executes only if no exception was found
# print('Done')
#%%
...
# %%
# Demonstrate writing to a binary file - pickle.dump(<obj>, <outfile>) # from pickle package
bands = [theBeatles, theRollingStones, pinkFloyd]
...
bands = [theBeatles, theRollingStones, pinkFloyd]
file = get_project_dir() / 'data/bands.bin'
