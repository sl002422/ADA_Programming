...
f = return_function('George Harrison', True)
print(f())
...
def band_details(f_to_decorate):
    ...
    return wrap
#%%
@band_details           # another way of saying something like 'songs = a_very_simple_decorator(songs)' from previous example
...
print(print_band.__name__)      # try it with and without @functools.wraps(f_to_decorate) in the decorator; to avoid such output we should use import functools (see above)
